...
class Agent:
    ...
    def __init__(self, feature_space, gamma=0.99):
        self.actor = Actor(feature_space)
    ...
    def get_action(self, x):
        ...
        out = self.actor(x)
        mean, std = out[0], torch.exp(out[1])
        probs = torch.distributions.Normal(mean, std)
        action = probs.sample()
        self.log_probs = probs.log_prob(action)
        action = torch.tanh(action).item()
        return action, None

    def update(self, reward, new_state, old_state, done, graph=False):
        ...
        (actor_loss + critic_loss).backward(retain_graph=graph)
        self.actor.opt.step()
